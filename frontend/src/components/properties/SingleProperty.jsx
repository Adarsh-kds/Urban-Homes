...
const SingleProperty = () => {
    ...
    async function bookmark() {
        setSave(!save)
        try {
            await axios.post(`https://house-rental-backend-p9wx.onrender.com/user/save/${params.id}`);
    ...
    const postData = async function () {
        try {
            const { data } = await axios.get(`https://house-rental-backend-p9wx.onrender.com/post/properties/${params.id}`);
            // console.log(data);
